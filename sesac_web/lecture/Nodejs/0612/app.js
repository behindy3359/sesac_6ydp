...
app.set('view engine','ejs'); // 세상에는 많은 뷰 엔진이 존재하지만 이번에는 이녀석을 사용할 예정
app.set('views', './views'); // view를 가져올 디렉토리를 설정, 목표 폴더 안에 존재하는 (.ejs)를 읽어온다
...
app.get('/', (req, res)=>{
    res.render('index')
    console.log('index 열림!');
})

app.listen(portNo, ()=>{
    console.log(portNo + '번 포트에서 서버 실행');
});
