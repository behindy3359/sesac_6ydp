...
// react hook form 은 언 컨트롤드 컴포넌트를 사용함. (컨트롤드 vs 언 컨트롤드)
// - 폼 필드가 업데이트 될 때마다 전체 폼이 리 렌더링 되지않도록 함 -> 최적화와 관련
...
// RHF 라이브러리
// - 코드의 간결화
// - 비제어 (언 컨트롤드) 컴포넌트 / 필요한 경우에만 리렌더링.
// - 다양한 검증 규칙이 이미 내장되어 있음
// - handlesubmit 함수가 모든 폼 데이터를 쉽게 관리 가능하게 함
// - 복잡하고 대규모 폼 양식을 작성할 때 사용.

// Q) 왜... 사용하는지?
// react 컴포넌트에서 ...연산자는 객체나 배열을 펼쳐서 각각의 요소나 속성을 개별적인 prop으로 전달할 때 사용합니다.
// 이 문법을 사용하면 코드가 간결해지고, 여러 개의 props를 한 번에 전달할 수 있습니다.

//props?
// - 컴포넌트에 전달되는 입력 값.
// - 부모 컴포넌트가 자식 컴포넌트에 데이터를 전달할 때 사용되며,
// 이 props를 통해 컴포넌트의 동작이나 출력이 결정됨

// react 에서는 객체의 속성을 컴포넌트의 props로 전달할 때, 각 속성을 개별적으로 전달할 수 있다.
